...
#define BTREE_FN(name)			BTREE_TP(pmem_btree_ ## name)
#define BTREE_TYPE_HEAD			BTREE_TP(struct pmem_btree_head)
#define VISITOR_FN			BTREE_TP(visitor)
#define VISITOR_FN_T			_BTREE_TP(visitor, BTREE_TYPE_SUFFIX, _t)

BTREE_TYPE_HEAD {
	struct pmem_btree_head h;
};
...
static inline int BTREE_FN(merge)(BTREE_TYPE_HEAD *target,
				  ...
				  gfp_t gfp)
{
	return pmem_btree_merge(&target->h, &victim->h, BTREE_TYPE_GEO, gfp);
}
...
static inline void *BTREE_FN(lookup)(BTREE_TYPE_HEAD *head, BTREE_KEYTYPE key)
{
	...
	return pmem_btree_lookup(&head->h, BTREE_TYPE_GEO, &_key);
}
...
{
	unsigned long _key = key;
	return pmem_btree_insert(&head->h, BTREE_TYPE_GEO, &_key, val, gfp);
}
...
{
	unsigned long _key = key;
	return pmem_btree_update(&head->h, BTREE_TYPE_GEO, &_key, val);
}

static inline void *BTREE_FN(remove)(BTREE_TYPE_HEAD *head, BTREE_KEYTYPE key)
{
	unsigned long _key = key;
	return pmem_btree_remove(&head->h, BTREE_TYPE_GEO, &_key);
}

static inline void *BTREE_FN(last)(BTREE_TYPE_HEAD *head, BTREE_KEYTYPE *key)
{
	unsigned long _key;
	void *val = pmem_btree_last(&head->h, BTREE_TYPE_GEO, &_key);
	if (val)
		*key = _key;
	return val;
}
...
{
	unsigned long _key = *key;
	void *val = pmem_btree_get_prev(&head->h, BTREE_TYPE_GEO, &_key);
	if (val)
		*key = _key;
	return val;
}
...
static inline void *BTREE_FN(lookup)(BTREE_TYPE_HEAD *head, BTREE_KEYTYPE key)
{
	return pmem_btree_lookup(&head->h, BTREE_TYPE_GEO, (unsigned long *)&key);
}

static inline int BTREE_FN(insert)(BTREE_TYPE_HEAD *head, BTREE_KEYTYPE key,
			   void *val, gfp_t gfp)
{
	return pmem_btree_insert(&head->h, BTREE_TYPE_GEO, (unsigned long *)&key,
			    val, gfp);
}
...
static inline int BTREE_FN(update)(BTREE_TYPE_HEAD *head, BTREE_KEYTYPE key,
		void *val)
{
	return pmem_btree_update(&head->h, BTREE_TYPE_GEO, (unsigned long *)&key, val);
}

static inline void *BTREE_FN(remove)(BTREE_TYPE_HEAD *head, BTREE_KEYTYPE key)
{
	return pmem_btree_remove(&head->h, BTREE_TYPE_GEO, (unsigned long *)&key);
}

static inline void *BTREE_FN(last)(BTREE_TYPE_HEAD *head, BTREE_KEYTYPE *key)
{
	return pmem_btree_last(&head->h, BTREE_TYPE_GEO, (unsigned long *)key);
}

static inline void *BTREE_FN(get_prev)(BTREE_TYPE_HEAD *head, BTREE_KEYTYPE *key)
{
	return pmem_btree_get_prev(&head->h, BTREE_TYPE_GEO, (unsigned long *)key);
}
#endif
...
static inline size_t BTREE_FN(visitor)(BTREE_TYPE_HEAD *head,
				       ...
				       VISITOR_FN_T func2)
{
	return pmem_btree_visitor(&head->h, BTREE_TYPE_GEO, opaque,
			     visitorl, func2);
}
...
static inline size_t BTREE_FN(grim_visitor)(BTREE_TYPE_HEAD *head,
					    ...
					    VISITOR_FN_T func2)
{
	return pmem_btree_grim_visitor(&head->h, BTREE_TYPE_GEO, opaque,
				  visitorl, func2);
}
