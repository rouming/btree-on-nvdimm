...
/**
 * pmem_btree_alloc - allocate function for the mempool
...
/**
 * pmem_btree_free - free function for the mempool
...
/**
 * pmem_btree_init_mempool - initialise a btree with given mempool
 *
...
/**
 * pmem_btree_init - initialise a btree
 *
 * @head: the btree head to initialise
 *
 ...
 * when using pmem_btree_init(), not with pmem_btree_init_mempool().
 */
void pmem_btree_destroy(struct pmem_btree_head *head);

/**
 * pmem_btree_lookup - look up a key in the btree
 *
 ...
 */
void *pmem_btree_lookup(struct pmem_btree_head *head,
			struct pmem_btree_geo *geo,
			unsigned long *key);

/**
 * pmem_btree_insert - insert an entry into the btree
 *
 ...
 */
int __must_check pmem_btree_insert(struct pmem_btree_head *head,
				   struct pmem_btree_geo *geo,
				   unsigned long *key, void *val, gfp_t gfp);
/**
 * pmem_btree_update - update an entry in the btree
 *
...
/**
 * pmem_btree_remove - remove an entry from the btree
 *
 * @head: the btree to update
 * @geo: the btree geometry
...
/**
 * pmem_btree_merge - merge two btrees
 *
...
/**
 * pmem_btree_last - get last entry in btree
 *
 * @head: btree head
 * @geo: btree geometry
 * @key: last key
 ...
 * key is not changed.
 */
void *pmem_btree_last(struct pmem_btree_head *head,
		      struct pmem_btree_geo *geo,
		      unsigned long *key);

/**
 * pmem_btree_get_prev - get previous entry
 *
 * @head: btree head
 * @geo: btree geometry
 * @key: pointer to key
...
/* internal use, use pmem_btree_visitor{l,32,64,128} */
size_t pmem_btree_visitor(struct pmem_btree_head *head,
			  struct pmem_btree_geo *geo,
...
/* internal use, use pmem_btree_grim_visitor{l,32,64,128} */
...
extern struct pmem_btree_geo pmem_btree_geo32;
#define BTREE_TYPE_SUFFIX l
#define BTREE_TYPE_BITS BITS_PER_LONG
#define BTREE_TYPE_GEO &pmem_btree_geo32
#define BTREE_KEYTYPE unsigned long
#include "pmem-btree-type.h"

#define pmem_btree_for_each_safel(head, key, val)	\
	for (val = pmem_btree_lastl(head, &key);	\
	     val;				\
	     val = pmem_btree_get_prevl(head, &key))
...
#define pmem_btree_for_each_safe32(head, key, val)	\
	for (val = pmem_btree_last32(head, &key);	\
	     val;				\
	     val = pmem_btree_get_prev32(head, &key))

extern struct pmem_btree_geo pmem_btree_geo64;
#define BTREE_TYPE_SUFFIX 64
#define BTREE_TYPE_BITS 64
#define BTREE_TYPE_GEO &pmem_btree_geo64
#define BTREE_KEYTYPE u64
#include "pmem-btree-type.h"

#define pmem_btree_for_each_safe64(head, key, val)	\
	for (val = pmem_btree_last64(head, &key);	\
	     val;				\
	     val = pmem_btree_get_prev64(head, &key))
